...
- **Architect** — uses the extension to capture items and the Mothership dashboard to manage projects/schedules. They care about specifity and clarity.
- **Contractor** — receives a share link and views the read-only schedule to buy items. They care about working as quickly as possible, and staying on or ahead of schedule, and on or under budget.
  ...
  server/                Go server (Mothership dashboard + extraction API)
    ...
    internal/
      ...
      db/
        ...
        generated/       sqlc-generated Go types + query functions (DO NOT edit; run
                         `cd server && sqlc generate` after changing queries or schema)
        admin_stats.go   Hand-written admin/analytics queries that use *sql.DB directly
                         (not part of the sqlc pipeline; package db, not generated)
...
2. From `server/`, run `sqlc generate` — output lands in `server/internal/db/generated/`
3. Do **not** hand-edit anything under `server/internal/db/generated/`. Regenerate instead.
4. If your query joins onto another table and you want the row embedded, use
   `select sqlc.embed(p), ...` rather than listing the foreign columns flat.

Install sqlc if missing: `go install github.com/sqlc-dev/sqlc/cmd/sqlc@v1.30.0`.

Note: `server/internal/db/admin_stats.go` (package `db`) is the home for analytics
queries that use `*sql.DB` directly — they live outside the sqlc pipeline because
they're admin-only and don't fit the per-table generated layout.
