...
- GitHub Actions uses the repository `development` environment on the self-hosted runner to prepare server artifacts for that development host. It does not perform an automatic rollout.
...
3. builds a `sally-server:dev` image and uploads both artifacts
...
The workflow currently prepares artifacts only; it does not automate rollout yet.
