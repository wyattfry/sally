...
func (a app) uploadItemImage(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("Content-Type", "text/html")
	writeItemThumbCell(w, pickerURL, item.ID, url)
}
...
// writeItemThumbCell writes the full <td> thumb fragment (for initial render and row rebuilds).
func writeItemThumbCell(w io.Writer, pickerURL, itemID, imageURL string) {
	e := html.EscapeString
	id := "item-thumb-" + itemID
	fmt.Fprintf(w, `<td class="item-thumb-cell upload-target" id="%s" title="Click to change image">`, e(id))
	writeItemThumbInner(w, pickerURL, imageURL)
	fmt.Fprint(w, `</td>`)
}

// writeItemThumbInner writes just the button inside the thumb cell.
// Used by pickItemImage for innerHTML swaps (avoids HTMX <td> parsing limitations).
func writeItemThumbInner(w io.Writer, pickerURL, imageURL string) {
	e := html.EscapeString
	var imgHTML string
	if imageURL != "" {
	...
	fmt.Fprintf(w,
		`<button type="button" class="item-thumb-btn" hx-get="%s" hx-target="body" hx-swap="beforeend">%s<div class="upload-overlay"><span>⊕</span></div></button>`,
		e(pickerURL), imgHTML)
}
